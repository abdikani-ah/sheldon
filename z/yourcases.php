...
$select = "select caschoices.id as notrem, * from caschoices where userid=:sessionid";
$stmt = $conn->prepare($select);
$stmt->bindParam(':sessionid', $_SESSION[id]);
$stmt->execute();
$result=$stmt->fetchAll();
if($_GET[del]){
$delete = "delete from caschoices where id=:getdel and userid=:sessionid";
$ab = $conn->prepare($delete);
$ab->bindParam(':sessionid', $_SESSION[id]);
$ab->bindParam(':getdel', $_GET[del]);
$ab->execute();
header('Location: yourcases.php');
}
$userid = $_SESSION[id];
$id = $_GET[del];
echo '<center>';
...
foreach($result as $x){
echo '
	<tr class="row"><td><a href=?del='.$x[notrem].' style="text-decoration: none;">Remove cas</a></td><td>'.$x['creativity'].'</td><td>'.$x['activity'].'</td><td>'.$x['service'].'</td><td>'.$x['studentjob'].'</td><td>'.$x['tprice'].'</td></tr>';
}
echo '</table>';
